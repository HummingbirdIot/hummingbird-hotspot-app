...
const BarChart = ({
  ...
  const hasDownBars = useMemo(
    () => some(allData, ({ down }) => down > 0),
    [allData],
  )
  ...
  const barWidth = useMemo(
    () => width / (data.length + data.length - 1),
    [width, data.length],
  )

  // min bar height is the same as bar width to form circles
  const minBarHeight = useMemo(
    () => barWidth / (hasStackedData ? 2 : 1),
    [barWidth, hasStackedData],
  )

  // the pixel value of the space between up and down bars
  const barGap = useMemo(
    () => (hasDownBars ? barWidth / 1.5 : 0),
    [hasDownBars, barWidth],
  )

  // raw max up value
  const maxUp = useMemo(
    () => maxBy(allData, 'up')?.up || minBarHeight,
    [allData, minBarHeight],
  )

  // raw max down value
  const maxDown = useMemo(
    () => maxBy(allData, 'down')?.down || minBarHeight,
    [allData, minBarHeight],
  )
